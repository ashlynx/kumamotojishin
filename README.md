...
ホームに出している断水戸数は**手で書いた固定値**です。国土交通省がPDFでしか出しておらず、機械的に追えないためです。**報が更新されたら手で直してください**（`site/index.html` の「断水」で検索）。最新報は https://www.mlit.go.jp/saigai/saigai_260728.html に並びます。

数字を載せるときは、**必ず「いつ時点か」を併記**してください。断水戸数は復旧で減り、被害判明で増えます。時点のない数字は誤解を生みます。
