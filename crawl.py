...
SOURCES = [
    ...
    # 【2026-09-20】宇土市の巡回をやめました。
    #   RSS（/rss/nrss.xml）は、ブラウザのUAで開いても403が返ります。配信そのものが
    #   止まっているようです。HTMLの一覧（/category/list/1301.html）は開けますが、
    #   短時間に何度か取りに行くと、そこも403になります（回数で弾かれています）。
    #   9月4日から16日間、毎回「取得失敗」を出し続けていました。
    #   9月30日で更新を終えるので、ここでHTML一覧を読む仕組みを作るのは割に合いません。
    #   宇土市の欄と市のページへのリンクはサイトに残してあるので、読む人はそちらへ行けます。
    #   戻すときは、この位置に kind:"rss" ではなく専用のHTMLパーサーで入れること。
    {
        "id": "hikawa",
        "label": "氷川町",
        "kind": "rss",
